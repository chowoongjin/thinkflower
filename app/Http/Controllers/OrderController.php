...
class OrderController extends Controller
{
    ...
    {
        $request->merge([
            'original_amount' => $request->filled('original_amount')
                ? (int) preg_replace('/[^0-9]/', '', (string) $request->input('original_amount'))
                : null,
            'order_amount' => $request->filled('order_amount')
                ? (int) preg_replace('/[^0-9]/', '', (string) $request->input('order_amount'))
                : null,
        ...
        $validated = $request->validate([
            ...
            'delivery_hour' => ['required', 'integer', 'between:0,23'],
            'delivery_minute' => ['required', 'integer', 'in:0,10,20,30,40,50'],
            'delivery_time_type' => ['required', 'in:도착,예식,행사'],
            'recipient_name' => ['required', 'string', 'max:50'],
            'recipient_phone' => ['required', 'string', 'max:30'],
            'ribbon_quick' => ['required', 'string', 'max:100'],
            ...
            'delivery_hour.required' => '배달요청 시간을 선택해 주세요.',
            'delivery_minute.required' => '배달요청 분을 선택해 주세요.',
            ...
            'ribbon_quick.required' => '간편문구를 선택해 주세요.',
        ...
        DB::transaction(function () use (
            ...
            $order = Order::create([
                ...
                'original_amount' => (int) ($validated['original_amount'] ?? 0),
